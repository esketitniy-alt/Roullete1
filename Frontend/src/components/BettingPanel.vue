      ...
      <div class="my-bets-list">
        ...
        >
          ...
          <span class="potential-win">
            → {{ bet.amount * store.multipliers[bet.color] }} 🪙
          </span>
        </div>
      </div>
      <div class="total-my-bets">
        <span>Всего поставлено: {{ totalMyBets }} 🪙</span>
      </div>
    </div>
    
    <!-- Предупреждение если не авторизован -->
...
const placeBet = (color) => {
  ...
  if (betAmount.value > store.maxBet) {
    store.showNotification(`Максимальная ставка: ${store.maxBet}`, 'error');
    return;
  }
...
.color-btn.red {
  background: linear-gradient(135deg, var(--red) 0%, #c73e54 100%);
}
